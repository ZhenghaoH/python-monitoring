...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        read_table_entries(p4info_helper, s1)
...
def read_table_entries(p4info_helper, switch):
    for table in p4info_helper.p4info.tables:
        table_name = table.preamble.name
        table_id = table.preamble.id
        print("\nReading entries from table '%s'" % table_name)
        entries = switch.ReadTableEntries(table_id=table_id)
        entry_count = len(list(entries))
        print("Table '%s' has %d entries" % (table_name, entry_count))
...
def sql_insert(swid, Band):
    try:
        with connection.cursor() as cursor:
            # insert
            add_data = ("INSERT INTO `sys`.`INT_data` (`switch_id`, `Bandwidth`)"
                            " VALUES (%s, %s) "
