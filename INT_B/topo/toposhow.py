...
# 可视化拓扑图
def draw_topology(G):
    pos = nx.spring_layout(G, seed=42)  # 使用spring layout布局
    # 提取主机和交换机节点
    ...
    # 绘制节点
    nx.draw_networkx_nodes(G, pos, nodelist=host_nodes, node_color='skyblue', node_shape='s', node_size=400, label='Hosts')
    nx.draw_networkx_nodes(G, pos, nodelist=switch_nodes, node_color='lightcoral', node_shape='o', node_size=400, label='Switches')
